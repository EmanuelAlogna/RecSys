...
cdef class SLIM_BPR_Cython_Epoch:
    ...
    cdef Triangular_Matrix SIM_ICM

    def __init__(self, URM_mask,ICM,
        ...
        if self.train_with_sparse_weights:
            ...
            self.S_symmetric = Triangular_Matrix(self.n_items, None, isSymmetric = True)

            self.SIM_ICM = Triangular_Matrix(self.n_items,  normalize(sps.csr_matrix.dot(ICM,ICM.T),norm='l2')
            ,isSymmetric= True)


        else:
            self.S_dense = np.zeros((self.n_items, self.n_items), dtype=np.float64)
            # self.ICM1 = sps.csr_matrix(ICM)
            # self.ICM2 = sps.csc_matrix(ICM)
    ...
    def epochIteration_Cython(self,ICM):
        ...
        cdef int[:] row_columns
        cdef double[:] data


        cdef int row_start2,row_end2
        cdef int[:] row_columns2
        cdef double[:] data2

        cdef int target_row
        cdef int target_row2
        ...
        print(totalNumberOfBatch)

        #SIM_ICM = sps.csr_matrix.dot(ICM,ICM.T).toarray()
        ...
        for numCurrentBatch in range(totalNumberOfBatch):
            ...
            while (index <  sample.seen_items_end_pos - sample.seen_items_start_pos):
                ...
                if self.train_with_sparse_weights:
                   x_uij += self.S_sparse.get_value(i, seenItem) - self.S_sparse.get_value(j, seenItem)
                    ...
                    x_uij += 0.8*(self.S_symmetric.get_value(i, seenItem) - self.S_symmetric.get_value(j, seenItem)) + 0.2*(self.SIM_ICM.get_value(i,seenItem) - self.SIM_ICM.get_value(j,seenItem))

                else:

                    x_uij += self.S_dense[i, seenItem] - self.S_dense[j, seenItem] + SIM_ICM[i,seenItem] - SIM_ICM[j,seenItem]
            ...
            while index < sample.seen_items_end_pos - sample.seen_items_start_pos:
                ...
                if self.train_with_sparse_weights:
                    ...
                    self.SIM_ICM.add_value(i,seenItem, self.learning_rate * ((gradient_update * self.SIM_ICM.get_value(seenItem,i))) - self.li_reg*self.SIM_ICM.get_value(i,seenItem))
                    self.SIM_ICM.add_value(seenItem,i, self.learning_rate * ((gradient_update * self.SIM_ICM.get_value(i,seenItem))) - self.li_reg*self.SIM_ICM.get_value(seenItem,i))
                    self.SIM_ICM.add_value(j,seenItem, -self.learning_rate * ((gradient_update * self.SIM_ICM.get_value(seenItem,j))) - self.li_reg*self.SIM_ICM.get_value(j,seenItem))
                    self.SIM_ICM.add_value(seenItem,i, -self.learning_rate * ((gradient_update * self.SIM_ICM.get_value(j,seenItem))) - self.li_reg*self.SIM_ICM.get_value(seenItem,j))

                    #
                    # self.ICM[i,:] += self.learning_rate * ((gradient_update * self.ICM[:,i] ) - self.li_reg*self.ICM[i,:])
                    # self.ICM[:,i] += self.learning_rate * ((gradient_update * self.ICM[i,:] ) - self.li_reg*self.ICM[:,i])
                    # self.ICM[j,:] -= self.learning_rate * ((gradient_update * self.ICM[:,j] ) - self.li_reg*self.ICM[j,:])
                    # self.ICM[:,j] -= self.learning_rate * ((gradient_update * self.ICM[j,:] ) - self.li_reg*self.ICM[:,j])
