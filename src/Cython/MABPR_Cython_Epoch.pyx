...
from src.Recommender_utils import similarityMatrixTopK, check_matrix
import numpy as np
cimport numpy as np
import time
import sys
...
cdef class MABPR_Cython_Epoch:
    ...
    cdef int[:] UF_indices, UF_indptr

    cdef Sparse_Matrix_Tree_CSR S_sparse
    cdef Sparse_Matrix_Tree_CSR ICM_Sparse
    ...
    def __init__(self, URM_mask,UF,ICM,n_features = None,
                 neg_items = None,
        ...
        super(MABPR_Cython_Epoch, self).__init__()
        ...
        self.URM_mask_indices = np.array(UF.indices, dtype=np.int32)
        self.URM_mask_indptr = np.array(UF.indptr, dtype=np.int32)

        self.UF_indices = np.array(URM_mask.indices, dtype=np.int32)
        self.UF_indptr = np.array(URM_mask.indptr, dtype=np.int32)


        if self.train_with_sparse_weights:
            self.S_sparse = Sparse_Matrix_Tree_CSR(self.n_users, self.n_features)

        elif self.symmetric:
            self.S_symmetric = Triangular_Matrix(self.n_users,self.n_features, isSymmetric = True)

        else:
            self.S_dense = np.random.randn(self.n_users,self.n_features).toarray()
        ...
        if sgd_mode=='adagrad':
            ...
            self.sgd_cache_I = np.zeros((self.n_features), dtype=np.float64)
    ...
    def epochIteration_Cython(self,ICM):
        ...
        cdef long i, j,u
        cdef long index, seenItem, numCurrentBatch, itemId,start_index
        ...
        for numCurrentBatch in range(totalNumberOfBatch):
            ...
            target_row = i
            row_start = ICM.indptr[target_row]
            row_end =ICM.indptr[target_row + 1]
            column = ICM.indices[row_start:row_end]
            for item in column:
                arr.add(item)
            target_row = j
            row_start = ICM.indptr[target_row]
            ...
            start_index=self.URM_mask_indptr[u]
            end = self.URM_mask_indptr[u+1]
            start = self.URM_mask_indptr[u]
            index = 0
            while (index <  end-start):
                seenItem = self.URM_mask_indices[start_index + index]

                if seenItem in arr:
                    update = ICM2[i,seenItem] - ICM2[j,seenItem]
                else:
                    update = 0
                index += 1
                if self.train_with_sparse_weights:
                    x_uij += self.S_sparse.get_value(u, seenItem) * update

                elif self.symmetric:
                    x_uij += self.S_symmetric.get_value(i, seenItem) - self.S_symmetric.get_value(j, seenItem)

                else:
                   # x_uij += self.S_dense[u,:] * (np.subtract(self.ICM[i,:] , self.ICM[j,:]))
                    pass
            gradient = exp(-x_uij) / (1 + exp(-x_uij)) * update
            loss += x_uij**2
            #print(gradient)
            ...
            while index < end-start:
                ...
                if self.train_with_sparse_weights:
                    ...
                    self.S_sparse.add_value(u, seenItem, self.learning_rate * (gradient_update - self.li_reg * self.S_sparse.get_value(u, seenItem)))
    ...
    cdef BPR_sample sampleBPR_Cython(self):
        ...
        if not self.neg_items:
            ...
            while numSeenItems == 0 or numSeenItems==1 or numSeenItems == self.n_items:

                sample.user = rand() % self.n_users

                sample.seen_items_start_pos = self.UF_indptr[sample.user]
                sample.seen_items_end_pos = self.UF_indptr[sample.user + 1]

                numSeenItems = sample.seen_items_end_pos - sample.seen_items_start_pos

            index = rand() % numSeenItems

            sample.pos_item = self.UF_indices[sample.seen_items_start_pos + index]

            while (True):

                # print('numseenItems {}'.format(numSeenItems))
                # print(index)
                # print(index2)
                index2 = rand() % numSeenItems
                if index2 != index :
                    break

            sample.neg_item = self.UF_indices[sample.seen_items_start_pos + index2]
        else:
            ...
            while (not negItemSelected):
                sample.pos_item = rand() % self.n_items

                sample.neg_item = rand() % self.n_items

                index = 0
                while index < numSeenItems and self.URM_mask_indices[sample.seen_items_start_pos + index]!=sample.neg_item and self.URM_mask_indices[sample.seen_items_start_pos + index]!=sample.pos_item:
                    index+=1
...
cdef class Triangular_Matrix:
    ...
    def __init__(self, long num_rows,long num_columns , int isSymmetric = False):
        ...
        for numRow in range(self.num_rows):
               ...
               for numCol in range(numRow+1):
                   self.row_pointer[numRow][numCol] = np.random.randn()
